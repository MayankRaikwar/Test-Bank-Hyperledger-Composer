...
describe('Starting tests for bank-net', function() {
  ...
  it('website title should be bank-net', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('bank-net');
    })
  });
  ...
  it('navbar-brand should be bank-net',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('bank-net');
    });
  });
